...
void main() {
  ...
  group('LinkPreviewService', () {
    ...
    test(
      'identifies image via magic bytes for application/octet-stream',
      () async {
        ...
        expect(result, isA<ImageLinkPreviewInfo>());
        expect(
          (result as ImageLinkPreviewInfo).imageType,
          isA<RasterImageType>(),
        );
      },
    );
    ...
    test('returns null for failed requests', () async {
      ...
      final response = http.StreamedResponse(const Stream.empty(), 404);
